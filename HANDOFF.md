...
status: 운영
updated: 2026-07-09
summary: 규약 3중 강제화(스킬+훅)+영어 미러 CI. 훅에 스코프 디스패처 이식 — 프로젝트 폴더면 그 HANDOFF만·루트면 전 프로젝트 표(workspace_lib). newest_file이 git 무시 파일(.gitignore)까지 제외 — 백그라운드 로그(gitignore된 로그 파일 등) 오탐 해소.
